...
def generator(x, isTrain=True, reuse=False):
    with tf.variable_scope('generator', reuse=reuse):
        ...
        # 1st hidden layer
        conv1 = tf.layers.conv2d_transpose(x, 64, [2, 1], strides=(1, 1), padding='valid')
        lrelu1 = lrelu(tf.layers.batch_normalization(conv1, training=isTrain), 0.2)

        # 2nd hidden layer
        conv2 = tf.layers.conv2d_transpose(lrelu1, 32, [2, 3], strides=(3, 3), padding='same')
        lrelu2 = lrelu(tf.layers.batch_normalization(conv2, training=isTrain), 0.2)

        # 3rd hidden layer
        conv3 = tf.layers.conv2d_transpose(lrelu2, 16, [4, 2], strides=(2, 1), padding='same')
...
def discriminator(x, isTrain=True, reuse=False):
    with tf.variable_scope('discriminator', reuse=reuse):
        # 1st hidden layer
        conv1 = tf.layers.conv2d(x, 16, [4, 3], strides=(2, 2), padding='same')
        lrelu1 = lrelu(conv1, 0.2)

        # 2nd hidden layer
        conv2 = tf.layers.conv2d(lrelu1, 32, [4, 2], strides=(2, 1), padding='same')
        lrelu2 = lrelu(tf.layers.batch_normalization(conv2, training=isTrain), 0.2)

        # 3rd hidden layer
        conv3 = tf.layers.conv2d(lrelu2, 64, [2, 3], strides=(3, 3), padding='same')
        lrelu3 = lrelu(tf.layers.batch_normalization(conv3, training=isTrain), 0.2)
...
for iter in range(S.shape[0] // batch_size):
    ...
    if iter % 209 == 0:
        saver.save(sess, 'ckpt/seiemic_DCGAN128.ckpt', global_step=iter + 1)
...
for ii in range(S.shape[0] //batch_size):
    ...
    print(ii)
features = Flat_feat
np.save('features_DCGAN_55wX128.npy', features)
...
result = np.reshape(pred_labels_kmeans,[950, 550])
plt.imshow(result)
plt.show()
sess.close()
# seismicGAN
